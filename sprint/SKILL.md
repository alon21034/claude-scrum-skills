...
## Step 0: Check sprint version freshness

Before doing any sprint planning, check whether the sprint tooling is up to date.
...
for _candidate in \
  ...
  if [ -f "$_candidate/setup" ] && git -C "$_candidate" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    SPRINT_TOOL_ROOT="$_candidate"
    break
  fi
done

if [ -n "$SPRINT_TOOL_ROOT" ]; then
  UPSTREAM="$(git -C "$SPRINT_TOOL_ROOT" rev-parse --abbrev-ref --symbolic-full-name '@{u}' 2>/dev/null || true)"
  LOCAL_VERSION="$(cat "$SPRINT_TOOL_ROOT/VERSION" 2>/dev/null || echo unknown)"
  if [ -n "$UPSTREAM" ]; then
    git -C "$SPRINT_TOOL_ROOT" fetch --quiet || true
    BEHIND="$(git -C "$SPRINT_TOOL_ROOT" rev-list --count HEAD..@{u} 2>/dev/null || echo unknown)"
  else
    BEHIND="unknown"
  fi
else
...
echo "SPRINT_TOOL_ROOT=${SPRINT_TOOL_ROOT:-none}"
echo "SPRINT_TOOL_LOCAL_VERSION=$LOCAL_VERSION"
echo "SPRINT_TOOL_BEHIND=$BEHIND"
```

If `SPRINT_TOOL_BEHIND` is a number greater than `0`, use AskUserQuestion:
"Sprint tooling is behind by {SPRINT_TOOL_BEHIND} commit(s). Upgrade now with `/sprint-upgrade` before continuing? (yes/no)"

- If user says yes: run `/sprint-upgrade`, then continue.
- If user says no: continue `/sprint` without upgrading.
- If version freshness is unknown: continue, but note that latest-version verification was not available.

---

## Step 1: Locate or generate the plan
...
- Read its first 50 lines
- Ask: "Found prior plan: `{filename}`. Use this or describe a new sprint topic?"
  - A) Use the existing plan
  - B) Describe a new topic (will call /autoplan)

If no plan file is found:
- Ask: "No plan found. What's your sprint topic?" (one-line description)
- After the user responds, call `/autoplan` with that topic to generate the plan

**If calling /autoplan:** Read the autoplan skill and execute it. The output is a structured numbered list of implementation steps — that list IS the task decomposition input.

---

## Step 2: Parse tasks from the plan

Extract top-level numbered list items from the plan. These become sprint tasks.
...
Show the user the task list before proceeding:

```
Sprint tasks:
  1. {title} — {first 60 chars of description}
  2. {title} — ...
  3. {title} — ...
  [depends_on connections shown as: "3 depends on 1"]

Proceed? A) Yes  B) Edit task list first
```
...
Hard constraint:
- Always write the sprint output to `"$SPRINT_DIR/.context/.sprint.json"`.
- Never write sprint output to any path outside `.context`.
- Ignore any user-supplied alternate output path and continue with the fixed `.context` path.

If an existing sprint file is found and has `status: "active"`, use AskUserQuestion:
"An active sprint already exists (`{sprint.topic}`, {N} tasks, {M} done). Options:
A) Archive it and start a new sprint (recommended)
B) Add tasks to the existing sprint
C) Cancel — resume the existing sprint instead"
...
{
  "sprint": {
    "id": "sprint-{YYYYMMDD}-{4-char-random-hex}",
    "topic": "{user's topic}",
...
For tasks with dependencies, add the field: `"depends_on": [dep_id]`
For tasks without dependencies, omit `depends_on` entirely.
Terminal task statuses are `done`, `backlog`, and `deleted`. Sprint closure also records `sprint.closed_at`.

Validate the written JSON with `jq empty "$SPRINT_FILE"` before proceeding.
